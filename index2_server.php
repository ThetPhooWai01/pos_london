...
session_start();
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // calculate_discount_price and Total
        $query = "UPDATE sell_data SET f_price = price - discount;";
        if (!$connection->query($query)) {
            die("ERROR: Query failed. " . $connection->error);
        }

        $query = "UPDATE sell_data SET total = f_price * qty;";
        if (!$connection->query($query)) {
            die("ERROR: Query failed. " . $connection->error);
        }

        $query = "UPDATE `$user_id` SET f_price = price - discount;";
        if (!$connection->query($query)) {
            die("ERROR: Query failed. " . $connection->error);
        }

        $query = "UPDATE `$user_id` SET total = f_price * qty;";
        if (!$connection->query($query)) {
            die("ERROR: Query failed. " . $connection->error);
        }
        //============================================
        ...
        <option value="ဖာ">ဖာ</option>
                ...
                echo '<td contenteditable="true">' . $row['voucher'] . '</td>';
                echo '<td contenteditable="true">' . $row['customer'] . '</td>';
