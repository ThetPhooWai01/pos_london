...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            $stmt = $connection->prepare("INSERT INTO `$user_id` (product_name, price, qty, total, tra_id, type,voucher) VALUES (?, ?, ?, ?, ?, ?,?)");
            ...
            $today = date("Y-m-d");
            $stmt = $connection->prepare("INSERT INTO sell_data (date, barcode, product_name, price, qty, total, tra_id, type, exp_debt, voucher) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssssssss", $today, $barcode, $product_name, $pieces_price, $qty, $pieces_price, $random, $type, $exp_debt, $voucher);
            if ($stmt->execute()) {
                $errorMessage = "update_data success";
            } else {
                $errorMessage = "Error: " . $stmt->error;
...
<script>
              ...
              <div class="pr-3 pl-3 pb-3 w-sm-100 w-lg-25"> <button type="button" class="btn btn-primary btn-sm submit" id="">Yuan</button></div>
            </div>
          </form>
