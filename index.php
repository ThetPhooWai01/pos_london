...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            $stmt = $connection->prepare("INSERT INTO `$user_id` (product_name, price, qty, total, tra_id, type,voucher,buy_type) VALUES (?,?, ?, ?, ?, ?, ?,?)");
            $stmt->bind_param("ssssssss", $product_name, $pieces_price, $qty, $pieces_price, $random, $type,$voucher,$buy_type);

            // $stmt = $connection->prepare("INSERT INTO sell_database (product_name, price, qty, total, tra_id, type,voucher) VALUES (?, ?, ?, ?, ?, ?,?)");
            // $stmt->bind_param("sssssss", $product_name, $pieces_price, $qty, $pieces_price, $random, $type,$voucher);
            ...
            $today = date("Y-m-d");
            $stmt = $connection->prepare("INSERT INTO sell_data (date, barcode, product_name, price, qty, total, tra_id, customer, bill, type, exp_debt, voucher,discount,buy_type) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
            $stmt->bind_param("ssssssssssssss", $today, $barcode, $product_name, $pieces_price, $qty, $pieces_price, $random, $customer, $payment, $type, $exp_debt, $orderNumber,$discount,$buy_type);
            if ($stmt->execute()) {
                $errorMessage = "update_data success";
            } else {
                $errorMessage = "Error: " . $stmt->error;
          ...
          <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            ...
            <li class="nav-item menu-open">
              ...
              <ul class="nav nav-treeview">
                ...
                <li class="nav-item">
                  <a href="pages/customer_payment.php" class="nav-link">
                    <i class="far fa-circle nav-icon"></i>
          ...
          <form id="dataForm">
            <table >
              <tr class="table-column label-mb">
                <td class="td-reduce-padding w-50"><p class="text-nowrap label">BarCode:</p></td>
                <td class="td-reduce-padding w-100"><input class="w-500" type="text" name="barcode" id="barcode" oninput="sendData()" autofocus></td>         
              </tr>

              <tr class="table-column label-mb">
                <td class="td-reduce-padding"><p class="text-nowrap label">Rate:</p></td>
                <td class="td-reduce-padding w-100"><input class="w-500" type="text" name="rate" id="rate"></td>         
              </tr>

            </table>
            <div class="d-md-flex justify-content-around align-items-left mb-3 mt-2">
              <div class="pr-3 pl-3 pb-3 w-sm-100 w-lg-50"><button type="button" class="btn btn-primary btn-sm submit" value="refresh" id="print">Print</button></div>
              <div class="pr-3 pl-3 pb-3 w-sm-100 w-lg-50"><button type="button" class="btn btn-primary btn-sm submit" value="refresh" id="reset">Delete</button></div>
            ...
            <div class="d-md-flex justify-content-around align-items-center mb-3 mt-2">
              
              <div class="pr-3 pl-3 pb-3 w-sm-100 w-lg-50"><button type="button" class="btn btn-primary btn-sm submit" id="" onclick="baht()">Baht</button></div>
              <div class="pr-3 pl-3 pb-3 w-sm-100 w-lg-50"> <button type="button" class="btn btn-primary btn-sm submit" id="" onclick="yuan()" >Yuan</button></div>
            </div>
              <!-- convert to Bath -->
              <!-- check_1 -->
              <?php 
            ...
            <script>
                ...
                button_print.addEventListener('click', function() {
                  window.location.href = 'index_mini_print.php';
                });
